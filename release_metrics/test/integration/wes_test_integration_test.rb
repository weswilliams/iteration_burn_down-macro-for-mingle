...
class Wes::WesTestIntegrationTest < Test::Unit::TestCase
  ...
  def test_macro_contents
    iteration_burn_down = ReleaseMetrics.new(nil, project(PROJECT_RESOURCE), nil)
    result = iteration_burn_down.execute
    assert result
  end
  
  def test_macro_contents_with_a_project_group
    iteration_burn_down = ReleaseMetrics.new(nil, [project(PROJECT_RESOURCE), project(ANOTHER_PROJECT_RESOURCE)], nil)
    result = iteration_burn_down.execute
    assert result
  end
